...
class FunctionManager:
  ...
  def get_function_info(self, func_name):
    # print("self.func_cache: ", self.func_cache)
  ...
  def _cache_function_parameters_and_return_type(self, tokenized_program):
    ...
    for line_num, line in enumerate(tokenized_program):
      if line and line[0] == InterpreterBase.FUNC_DEF:
        ...
        # format:  func funcname self.p1:t1 p2:t2 p3:t3 ...
        name_for_lambda = self.create_lambda_name(line_num)
        args = [self._to_tuple(x) for x in line[1:-1]]
        func_info = FuncInfo(args, line_num + 1)  # function starts executing on line after funcdef
        self.func_cache[name_for_lambda] = func_info
        # print("yes: ",  self.func_cache)
